const constants = {
    // parking
    checkExit: "SELECT * FROM parking_status WHERE slot = $1 AND status = $2",
    ...
    getStatus: "SELECT ps.slot, CASE ps.status WHEN 0 THEN 'ว่าง' WHEN 1 THEN 'ไม่ว่าง' WHEN 2 THEN 'ซ่อมบำรุง'END AS status FROM parking_status ps ORDER BY id",
    ...
    getStatusCar: "SELECT * FROM parking_status WHERE slot = $1",
